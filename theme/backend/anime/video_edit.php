...
<div class="row">
	<div class="col-12">
		<div class="card">
			<div class="card-body">
				<form action="<?= base_url("admin/anime/video/edit/$row->id/$rws->id") ?>" method="post">
					...
					<div class="tab-content">
						<div class="tab-pane show active" id="genereal">
							...
							<div class="mb-3">
								...
								<select class="form-select" name="type" id="av_type" onchange="typeav()">
									<option <?= ($rws->type == 'series') ? "selected" : "" ?> value="series">Series</option>
									<option <?= ($rws->type == 'movie') ? "selected" : "" ?> value="movie">Movie</option>
								</select>
							</div>
							<div class="mb-3" id="series">
								<label for="simpleinput" class="form-label">Episode</label>
								<input type="number" name="series" value="<?= $rws->eps ?>" class="form-control">
							</div>
						</div>
						<div class="tab-pane" id="videos">
							...
							<div id="dynamic_videos">
								<div class="mb-3">
									...
									<textarea name="videos[]" class="form-control" placeholder="embed"><?= explode('{{==}}', $rws->video)[0] ?></textarea>
								</div>
							</div>
						</div>
						<div class="tab-pane" id="downloads">
							...
							<div id="dynamic_downloads">
								<div class="mb-3">
									<label class="form-label">Server 1</label>
									<textarea name="downloads[]" class="form-control" placeholder="embed"><?= explode('{{==}}', $rws->download)[0] ?></textarea>
								</div>
							</div>
						</div>
					</div>
					<div class="d-flex justify-content-end">
						<button type="submit" name="btn" value="eps" class="btn btn-success">Update</button>
					</div>
				</form>
			</div>
		</div>
...
<script>
	...
	$(document).ready(function() {
		...
		<?php
		if (count(explode('{{==}}', $rws->video)) >= 1) {
			for ($x = 1; $x < count(explode('{{==}}', $rws->video)); $x++) { ?>
				i++;
				$('#dynamic_videos').append('<div class="mb-3" id="videos' + i + '"><label class="form-label">Server ' + i + ' <a class="remove_videos" id="' + i + '" href="javascript:void(0)"><i class="uil-trash-alt"></i></a></label><textarea name="videos[]" class="form-control" placeholder="embed"><?= explode('{{==}}', $rws->video)[$x] ?></textarea></div>');
		<?php }
		} ?>
		<?php
		if (count(explode('{{==}}', $rws->download)) >= 1) {
			for ($x = 1; $x < count(explode('{{==}}', $rws->download)); $x++) { ?>
				ii++;
				$('#dynamic_downloads').append('<div class="mb-3" id="downloads' + ii + '"><label class="form-label">Server ' + ii + ' <a class="remove_downloads" id="' + ii + '" href="javascript:void(0)"><i class="uil-trash-alt"></i></a></label><textarea name="videos[]" class="form-control" placeholder="embed"><?= explode('{{==}}', $rws->download)[$x] ?></textarea></div>');
		<?php }
		} ?>
	});
</script>